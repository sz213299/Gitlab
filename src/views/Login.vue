<template>
    <div class="login">
      ...
      <el-form ref="loginRef" :model="loginForm" :rules="loginRules" class="login-form">
        <h3 class="title">舟哥Django后台管理系统</h3>
...
//   规则账号密码不能为空
  ...
  const handleLogin=()=>{
    loginRef.value.validate(async(valid)=>{
        if(valid){
              // await 处理请求
        let result = await requsetUtil.post("user/login?"+qs.stringify(loginForm.value))
        console.log(result)
        let data =result.data
        if (data.code==200){
            ElMessage.success(data.info)
            // token 存进去sessionStorage
            window.sessionStorage.setItem("token",data.token)
            // Json.stringify将json对象转化成json字符串  json.parse 将json字符串转化成json对象
            window.sessionStorage.setItem("currenUser",JSON.stringify(data.user))

            window.sessionStorage.setItem("menuList",JSON.stringify(data.menuList))



            // 勾选了需要设置密码在 cookie 中设置记住用户名和密码
            // expires 有效期30天
  ...
  function getCookie(){
    ...
    loginForm.value={
        ...
        password:password === undefined ? loginForm.value.password : decrypt(username),
